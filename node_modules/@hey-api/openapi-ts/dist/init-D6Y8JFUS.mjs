...
import { ConfigError, OperationPath, OperationStrategy, applyNaming, buildDiscriminatedUnion, buildSymbolIn, childContext, coerce, createOperationKey, createSchemaProcessor, createSchemaWalker, deduplicateSchema, defineConfig, definePluginConfig, ensureDirSync, escapeComment, findTsConfigPath, getBaseUrl, getInput, getLogs, getParser, hasOperationDataRequired, hasParameterGroupObjectRequired, isEnvironment, operationPagination, operationResponsesMap, outputHeaderToPrefix, parseUrl, pathToJsonPointer, pathToName, refToName, requestValidatorLayers, resolvePlugins, resolveValidatorLayer, satisfies, sourceConfig, statusCodeToGroup, toCase } from "@hey-api/shared";
...
*/
function getProjectDependencies(configFilePath) {
...
function ANGULAR(factory) {
	return {
		...
		Injectable: factory.register("Injectable", { external: "@angular/core" }),
		httpResource: factory.register("httpResource", { external: "@angular/common/http" }),
		inject: factory.register("inject", { external: "@angular/core" })
	};
}
//#endregion
//#region src/symbols/arktype.ts
function ARKTYPE(factory) {
	return { type: factory.register("type", { external: "arktype" }) };
}
//#endregion
...
//#endregion
//#region src/symbols/faker.ts
function FAKER(factory, locale) {
	return {
...
//#endregion
//#region src/symbols/fastify.ts
function FASTIFY(factory) {
	return { RouteHandler: factory.register("RouteHandler", {
		external: "fastify",
		kind: "type"
	}) };
}
//#endregion
//#region src/symbols/msw.ts
function MSW(factory) {
	return {
		...
		HttpResponse: factory.register("HttpResponse", { external: "msw" }),
		...
		http: factory.register("http", { external: "msw" })
	};
}
//#endregion
//#region src/symbols/orpc-contract.ts
function ORPC_CONTRACT(factory) {
	return { oc: factory.register("oc", { external: "@orpc/contract" }) };
}
//#endregion
//#region src/symbols/pinia-colada.ts
function PINIA_COLADA(factory) {
	return {
		...
		defineQueryOptions: factory.register("defineQueryOptions", { external: "@pinia/colada" })
	};
}
//#endregion
//#region src/symbols/swr.ts
function SWR(factory) {
	return { useSWR: factory.register("useSWR", {
		external: "swr",
		importKind: "default",
		kind: "function"
	}) };
...
//#endregion
//#region src/symbols/temporal-polyfill.ts
function TEMPORAL_POLYFILL(factory) {
	return { Temporal: factory.register("Temporal", { external: "temporal-polyfill" }) };
}
//#endregion
...
//#endregion
//#region src/plugins/@angular/common/imports.ts
function angularImports(plugin) {
	const factory = plugin.symbolFactory;
	return { ...ANGULAR(factory) };
}
//#endregion
//#region src/plugins/@angular/common/http-requests/resolve.ts
...
//#endregion
//#region src/plugins/@angular/common/http-resources/resolve.ts
...
function operationAuth({ operation, plugin }) {
	...
	for (const securitySchemeObject of operation.security) {
		...
		else if (securitySchemeObject.type !== "mutualTLS") console.warn(`❗️ SDK warning: unsupported security scheme. Please open an issue if you'd like it added https://github.com/hey-api/hey-api/issues\n${JSON.stringify(securitySchemeObject, null, 2)}`);
	}
	return auth;
}
//#endregion
...
function operationStatements({ isRequiredOptions, opParameters, operation, plugin }) {
	...
	if (hasParams) {
		...
		statements.push($.const("params").assign($(plugin.imports.buildClientParams).call($.array(...args), $.array($.object().prop("args", $.array(...config))))));
...
function createShell$1(plugin) {
	...
	return { define: (node) => {
		...
		}))).$if(isAngularClient && node.isRoot, (c) => c.decorator(plugin.imports.angular.Injectable, $.object().prop("providedIn", $.literal("root"))));
		const dependencies = [];
...
function implementFn(args) {
	...
	return node.$if(isNuxtClient, (m) => m.generic(nuxtTypeComposable, (t) => t.extends(plugin.imports.Composable).default($.type.literal("$fetch"))).generic(nuxtTypeDefault, (t) => t.$if(plugin.querySymbol({
		category: "type",
		resource: "operation",
		resourceId: operation.id,
		role: "response"
...
function createHttpRequestShell(plugin) {
	...
	return { define: (node) => {
		...
		return {
			dependencies: [],
			node: $.class(symbol).export().$if(isAngularClient && node.isRoot, (c) => c.decorator(plugin.imports.Injectable, $.object().prop("providedIn", $.literal("root"))))
		};
	} };
}
...
function implementHttpRequestFn(args) {
	...
	return node.param("options", (p) => p.required(isRequiredOptions).type($.type(symbolOptions).generic(symbolDataType ?? "unknown").generic("ThrowOnError"))).generic("ThrowOnError", (g) => g.extends("boolean").default(false)).returns($.type(plugin.imports.HttpRequest).generic(symbolResponseType ?? "unknown")).do($.return($("options").attr("client").optional().$if(symbolClient, (c, s) => c.coalesce(s)).attr("requestOptions").call($.object().prop("responseStyle", $.literal("data")).prop("method", $.literal(operation.method.toUpperCase())).prop("url", $.literal(operation.path)).spread("options")).generic(symbolResponseType ?? "unknown").generic("ThrowOnError")));
}
function implementHttpResourceFn(args) {
	...
	return node.param("options", (p) => p.required(isRequiredOptions).type($.type.func().returns($.type.or($.type(symbolOptions).generic(symbolDataType ?? "unknown").generic("ThrowOnError"), $.type("undefined"))))).generic("ThrowOnError", (g) => g.extends("boolean").default(false)).do($.return($(plugin.imports.httpResource).call($.func().do($.const("opts").assign($.ternary("options").do($("options").call()).otherwise($.id("undefined"))), $.return($.ternary("opts").do($.lazy((ctx) => ctx.access(plugin.referenceSymbol(createHttpRequestFnMeta(operation)), { transform: (node, index) => index === 0 ? node["~dsl"] === "ClassTsDsl" ? $(plugin.imports.inject).call($(node.name)) : $(node.name) : node }).call("opts"))).otherwise($.id("undefined"))))).generic(symbolResponseType ?? "unknown")));
}
...
const defaultConfig$28 = {
	...
	name: "@angular/common",
	symbolMeta() {
		return { artifact: "@angular/common" };
	}
};
definePluginConfig(defaultConfig$28);
//#endregion
//#region src/plugins/@faker-js/faker/shared/helpers.ts
...
//#endregion
//#region src/plugins/@faker-js/faker/shared/faker-expr.ts
/**
...
*/
function createFakerWalkerContext(tracking, circularPointers, isCircularSchema, nameRules) {
...
function mapRules(overrides) {
	const compoundRules = {};
	const exactMatchRules = {};
	const suffixRules = [];
	Object.entries(overrides).forEach(([key, { suffixMatch, ...rule }]) => {
		if (key.includes(".")) compoundRules[normalizeName(key)] = rule;
		else if (suffixMatch) suffixRules.push({
			rule,
			suffix: normalizeName(key)
		});
		else exactMatchRules[normalizeName(key)] = rule;
	});
	return {
...
//#endregion
//#region src/plugins/@faker-js/faker/v10/toAst/array.ts
/**
* Generates `(options?.faker ?? faker).helpers.multiple(() => <item expr>)`.
*/
function arrayToExpression({ fakerCtx, schema, walk, walkerCtx }) {
	...
	if (normalizedSchema.items && normalizedSchema.items.length > 0) {
		const result = walk(normalizedSchema.items.length > 1 ? {
			items: normalizedSchema.items,
			logicalOperator: normalizedSchema.logicalOperator
		} : normalizedSchema.items[0], childContext(walkerCtx, "items", 0));
		if (result.resultType === "never") return {
			expression: $.array(),
	...
	} else itemExpr = $("undefined");
	const callback = $.func().arrow().do($.return(itemExpr));
	let arrayExpr;
	if (schema.minItems !== void 0 || schema.maxItems !== void 0) {
	...
	} else arrayExpr = fakerCtx.fakerAccessor.attr("helpers").attr("multiple").call(callback);
	if (hasCircularRef) {
...
//#endregion
//#region src/plugins/@faker-js/faker/v10/toAst/boolean.ts
/**
* Generates `(options?.faker ?? faker).datatype.boolean()`.
*/
function booleanToExpression(ctx) {
	return ctx.fakerAccessor.attr("datatype").attr("boolean").call();
}
//#endregion
//#region src/plugins/@faker-js/faker/v10/toAst/enum.ts
/**
* Generates `(options?.faker ?? faker).helpers.arrayElement([...values])`.
*/
function enumToExpression(ctx, schema) {
	const members = [];
	for (const item of schema.items ?? []) if (item.const === null || item.type === "null") members.push($.fromValue(null));
	else if (item.const !== void 0) members.push($.literal(item.const));
	if (!members.length) return $("undefined");
	if (members.length === 1 && members[0]) return members[0];
	return ctx.fakerAccessor.attr("helpers").attr("arrayElement").call($.array(...members));
}
//#endregion
//#region src/plugins/@faker-js/faker/v10/toAst/nameRules.ts
const STRING_NAME_RULES = {
...
const STRING_SUFFIX_RULES = [
	{
		rule: { fakerPath: ["string", "uuid"] },
		suffix: "Id"
	},
	{
		rule: { fakerPath: ["internet", "email"] },
		suffix: "Email"
	},
	{
		rule: { fakerPath: ["internet", "url"] },
		suffix: "Url"
	},
	{
		rule: { fakerPath: ["phone", "number"] },
		suffix: "Phone"
	}
];
const NUMBER_NAME_RULES = {
	age: {
		defaultArgs: {
			max: 120,
			min: 1
		},
		fakerPath: ["number", "int"]
	},
	amount: {
		defaultArgs: {
			max: 1e4,
...
function propertyNameFromPath(path) {
	for (let i = path.length - 1; i >= 1; i--) if (path[i - 1] === "properties" && typeof path[i] === "string") {
		const name = path[i];
		const parentSegment = i >= 2 ? path[i - 2] : void 0;
		return {
			name,
			parent: typeof parentSegment === "string" && parentSegment !== "properties" ? parentSegment : void 0
		};
	}
}
/**
...
*/
function buildFromRule(ctx, rule, schemaArgs) {
	const fakerMethod = ctx.fakerAccessor.attr(rule.fakerPath[0]).attr(rule.fakerPath[1]);
	const merged = (rule.defaultArgs || schemaArgs) && typeof rule.defaultArgs === "object" ? {
		...rule.defaultArgs,
		...schemaArgs
	} : rule.defaultArgs || schemaArgs;
	if (merged) return $(fakerMethod).call($.fromValue(merged));
	return $(fakerMethod).call();
}
/** Look up compound key first, then standalone key. User overrides take priority. */
function lookupRule(nameInfo, standaloneMap, compoundMap, nameRuleOverrides) {
	...
	if (nameInfo.parent) {
		...
		if (nameRuleOverrides?.compoundRules) {
			...
			if (computeRuleOverride) return {
				isUserOverride: true,
				rule: computeRuleOverride
			};
		}
		const compoundRule = compoundMap[compoundKey];
		if (compoundRule) return {
...
*/
function stringNameToExpression(ctx, nameInfo) {
	...
	if (result) return buildFromRule(ctx, result.rule);
	if (nameRuleOverrides?.suffixRules) {
		const suffixRuleOverride = nameRuleOverrides.suffixRules.find((r) => nameInfo.name.endsWith(r.suffix));
		if (suffixRuleOverride) return buildFromRule(ctx, suffixRuleOverride.rule);
	}
	const suffixRule = STRING_SUFFIX_RULES.find((r) => nameInfo.name.endsWith(r.suffix));
	if (suffixRule) return buildFromRule(ctx, suffixRule.rule);
}
/**
...
* Returns `undefined` when no rule matches.
*/
function numberNameToExpression(ctx, nameInfo, schemaArgs) {
	const nameRuleOverrides = ctx.nameRulesByType.number;
	const result = lookupRule(nameInfo, NUMBER_NAME_RULES, {}, nameRuleOverrides);
	if (result) return buildFromRule(ctx, result.rule, result.isUserOverride ? void 0 : schemaArgs);
	if (nameRuleOverrides?.suffixRules) {
		const suffixRuleOverride = nameRuleOverrides.suffixRules.find((r) => nameInfo.name.endsWith(r.suffix));
		if (suffixRuleOverride) return buildFromRule(ctx, suffixRuleOverride.rule);
	}
...
//#endregion
//#region src/plugins/@faker-js/faker/v10/toAst/number.ts
/**
...
*/
function numberToExpression(ctx, schema, nameInfo) {
	const isInteger = schema.type === "integer";
	const method = isInteger ? "int" : "float";
	let min;
	let max;
	if (schema.exclusiveMinimum !== void 0) min = isInteger ? schema.exclusiveMinimum + 1 : schema.exclusiveMinimum;
	else if (schema.minimum !== void 0) min = schema.minimum;
	if (schema.exclusiveMaximum !== void 0) max = isInteger ? schema.exclusiveMaximum - 1 : schema.exclusiveMaximum;
	else if (schema.maximum !== void 0) max = schema.maximum;
	if (nameInfo) {
		const schemaArgs = {};
		if (min !== void 0) schemaArgs.min = min;
		if (max !== void 0) schemaArgs.max = max;
		const nameExpr = numberNameToExpression(ctx, nameInfo, schemaArgs);
		if (nameExpr) return nameExpr;
	}
	if (min !== void 0 || max !== void 0) {
		const options = $.object();
		if (min !== void 0) options.prop("min", $.literal(min));
		if (max !== void 0) options.prop("max", $.literal(max));
		return ctx.fakerAccessor.attr("number").attr(method).call(options);
	}
	return ctx.fakerAccessor.attr("number").attr(method).call();
}
//#endregion
//#region src/plugins/@faker-js/faker/v10/toAst/object.ts
/**
...
*/
function objectToExpression({ fakerCtx, schema, walk, walkerCtx }) {
	...
	for (const name in schema.properties) {
		...
		} else if (result.resultType === "never") continue;
		else {
			hasEmittedProperty = true;
			fakerCtx.tracking.needsResolveCondition = true;
			usesAccessor = true;
			const includeCondition = $("resolveCondition").call($.binary($(fakerCtx.optionsId).attr("includeOptional").optional(), "??", $.literal(true)), fakerCtx.fakerAccessor);
			let skipCondition;
			if (result.hasCircularRef) {
				fakerCtx.tracking.needsMaxCallDepth = true;
				const depthExceeded = $("_callDepth").gt($("MAX_CALL_DEPTH"));
				skipCondition = $.binary(depthExceeded, "||", $.prefix(includeCondition).not());
			} else skipCondition = $.prefix(includeCondition).not();
	...
	if (!hasEmittedProperty && schema.additionalProperties) {
		...
		if (result.resultType !== "never" && result.resultType !== "unknown") {
			...
			expression = $.ternary(skipCondition).do($.object().as("{}")).otherwise($.object().pretty().prop("additionalProp", result.expression));
		}
	}
	return {
		expression,
...
//#endregion
//#region src/plugins/@faker-js/faker/v10/toAst/string.ts
/**
...
*/
function stringToExpression(ctx, schema, nameInfo) {
	const formatExpr = formatToExpression(ctx, schema.format);
	if (formatExpr) return formatExpr;
	if (schema.pattern) return ctx.fakerAccessor.attr("helpers").attr("fromRegExp").call($.literal(schema.pattern));
	if (nameInfo) {
	...
	if (schema.minLength !== void 0 || schema.maxLength !== void 0) {
		...
		return ctx.fakerAccessor.attr("string").attr("alpha").call($.object().prop("length", lengthObj));
	}
	return ctx.fakerAccessor.attr("string").attr("sample").call();
}
function formatToExpression(ctx, format) {
	switch (format) {
		case "date": return ctx.fakerAccessor.attr("date").attr("recent").call().attr("toISOString").call().attr("slice").call($.literal(0), $.literal(10));
		case "date-time": return ctx.fakerAccessor.attr("date").attr("recent").call().attr("toISOString").call();
		case "email": return ctx.fakerAccessor.attr("internet").attr("email").call();
		case "ipv4": return ctx.fakerAccessor.attr("internet").attr("ipv4").call();
		case "ipv6": return ctx.fakerAccessor.attr("internet").attr("ipv6").call();
		case "uri":
		case "url": return ctx.fakerAccessor.attr("internet").attr("url").call();
		case "uuid": return ctx.fakerAccessor.attr("string").attr("uuid").call();
		case "binary": return $.new("Blob").arg($.array(ctx.fakerAccessor.attr("image").attr("dataUri").call()));
		default: return;
	}
}
//#endregion
//#region src/plugins/@faker-js/faker/v10/walker.ts
const FALLBACK_UNDEFINED = {
	expression: $("undefined"),
	isObjectLike: false,
	usesAccessor: false,
	usesFaker: false
...
*/
function isDiscriminatorConstraint(schema) {
	if (schema.type !== "object" || !schema.properties) return false;
	const props = schema.properties;
	return Object.keys(props).every((key) => props[key].const !== void 0);
}
function createVisitor$5(config) {
	const { circularPointers, isCircularSchema, plugin, schemaExtractor, tracking } = config;
	const fakerCtx = createFakerWalkerContext(tracking, circularPointers, isCircularSchema, plugin.config.nameRules);
	return {
		applyModifiers(result) {
			return result;
		},
		array(schema, ctx, walk) {
			const result = arrayToExpression({
				fakerCtx,
				schema,
				walk,
				walkerCtx: ctx
		...
		enum(schema) {
			...
			const needsFaker = (schema.items?.filter((item) => item.const !== void 0 || item.type === "null").length ?? 0) > 1;
			return {
				expression,
				isObjectLike: false,
		...
		integer(schema, ctx) {
			return {
				expression: numberToExpression(fakerCtx, schema, propertyNameFromPath(fromRef(ctx.path))),
				isObjectLike: false,
				usesAccessor: true,
				usesFaker: true
			};
		...
		intersection(items, schemas) {
			if (items.length === 0) return FALLBACK_UNDEFINED;
			if (items.length === 1 && items[0]) return items[0];
			if (!items.every((item) => item.isObjectLike)) {
				if (new Set(schemas.map((s) => {
					if (s.$ref || s.type === "object") return "object";
					if (s.items && s.logicalOperator) return "composite";
					return s.type;
				})).size > 1) return {
					...FALLBACK_UNDEFINED,
					resultType: "never"
			...
			for (const entry of indexed) if (isDiscriminatorConstraint(entry.schema)) discriminators.push(entry);
			else regular.push(entry);
			const assignCall = $("Object").attr("assign").call($.object(), ...regular.map(({ item }) => item.expression));
			let expression;
			if (discriminators.length > 0) {
				const obj = $.object().pretty();
				obj.spread(assignCall);
				for (const { schema } of discriminators) for (const key of Object.keys(schema.properties)) obj.prop(key, $($.fromValue(schema.properties[key].const)).as("const"));
				expression = obj;
			} else expression = assignCall;
			return {
				expression,
		...
		postProcess(result, schema) {
			if (schema.default !== void 0 && result.usesFaker) {
				tracking.needsResolveCondition = true;
				const condition = $("resolveCondition").call($.binary($(fakerCtx.optionsId).attr("useDefault").optional(), "??", $.literal(false)), fakerCtx.fakerAccessor);
		...
		reference($ref, _schema, ctx) {
			...
			return {
				expression: isCircularRef ? $(refSymbol).call(fakerCtx.optionsId, $("_callDepth").plus($.literal(1))) : refUsesFaker ? $(refSymbol).call(fakerCtx.optionsId) : $(refSymbol).call(),
		...
		string(schema, ctx) {
			return {
				expression: stringToExpression(fakerCtx, schema, propertyNameFromPath(fromRef(ctx.path))),
				isObjectLike: false,
				usesAccessor: true,
				usesFaker: true
			};
		...
		union(items, schemas) {
			...
			if (viableItems.length === 0) return {
				...FALLBACK_UNDEFINED,
				resultType: "never"
			};
			if (viableItems.length === 1 && viableItems[0]) return viableItems[0];
			const nullIndex = schemas.filter((_, i) => items[i]?.resultType !== "never").findIndex((s) => s.type === "null");
			...
			if (hasNull && nonNullItems.length === 1) {
				let expression = $.ternary(fakerCtx.fakerAccessor.attr("datatype").attr("boolean").call()).do(nonNullItems[0].expression).otherwise($.fromValue(null));
				if (nonNullItems[0].hasCircularRef) {
					fakerCtx.tracking.needsMaxCallDepth = true;
					expression = $.ternary($("_callDepth").gt($("MAX_CALL_DEPTH"))).do($.fromValue(null)).otherwise(expression);
				}
				return {
					expression,
			...
			if (hasNull) possibleValues.push($.fromValue(null));
			let expression = $(fakerCtx.fakerAccessor.attr("helpers").attr("arrayElement").call($.array(...possibleValues)).as("any"));
...
//#endregion
//#region src/plugins/@faker-js/faker/v10/api.ts
function toNodeV10({ plugin, schema }) {
	const visitor = createVisitor$5({
		circularPointers: /* @__PURE__ */ new Set(),
		isCircularSchema: false,
		plugin,
		tracking: {
			isObjectByRef: /* @__PURE__ */ new Map(),
			needsMaxCallDepth: false,
			needsResolveCondition: false,
			usesFakerByRef: /* @__PURE__ */ new Map()
		}
	});
	const result = createSchemaWalker(visitor)(schema, {
		path: ref([]),
		plugin
	});
	return visitor.applyModifiers(result, {
		path: ref([]),
		plugin
	}).expression;
}
function toNodeRefV10({ plugin, schema }) {
	if (schema.$ref) return $(plugin.referenceSymbol({
		artifact: "@faker-js/faker",
		category: "schema",
		resource: "definition",
...
var Api$4 = class {
	toNode(args) {
		const { plugin } = args;
		switch (plugin.config.compatibilityVersion) {
			default: return toNodeV10(args);
		}
	}
	toNodeRef(args) {
		const { plugin } = args;
		switch (plugin.config.compatibilityVersion) {
			default: return toNodeRefV10(args);
		}
	}
};
//#endregion
//#region src/plugins/@faker-js/faker/imports.ts
function fakerImports(plugin) {
	const factory = plugin.symbolFactory;
	const locale = plugin.config.locale;
	return { ...FAKER(factory, locale) };
}
//#endregion
//#region src/plugins/@faker-js/faker/shared/operation.ts
function isVoidSchema(schema) {
	return schema.type === "void" || schema.type === "never";
}
function collectMeaningfulResponses(responses, errors) {
	const result = [];
	if (responses?.properties) {
		for (const [code, schema] of Object.entries(responses.properties)) if (!isVoidSchema(schema)) result.push({
			group: "responses",
			schema,
			statusCode: code
		});
	...
	if (errors?.properties) {
		for (const [code, schema] of Object.entries(errors.properties)) if (!isVoidSchema(schema)) result.push({
			group: "errors",
			schema,
			statusCode: code
		});
...
*/
function suffixedNaming(naming, statusCode) {
	return {
		...naming,
		name: typeof naming.name === "function" ? (n) => naming.name(n) + statusCode : (naming.name ?? "") + statusCode
	};
}
/**
...
*/
function exportOperationResponse({ group, naming, operation, path, plugin, processor, schema, statusCode, tags }) {
	...
	const symbol = plugin.symbol(buildSymbolIn({
		meta: {
			category: "schema",
			path: [
				...path,
	...
	const statusLiteral = /^\d+$/.test(statusCode) ? $.type.literal(Number(statusCode)) : $.type.literal(statusCode);
	const arrowFn = $.func().arrow().$if(result.usesFaker, (f) => f.param("options", (p) => p.optional().type("Options"))).$if(typeSymbol, (f) => f.returns($.type(typeSymbol).idx(statusLiteral))).$if(result.usesAccessor, (f) => {
		const fDecl = $.const("f").assign($.binary($("options").attr("faker").optional(), "??", $(plugin.imports.faker)));
...
*/
function exportUnsuffixedResponse({ naming, operation, path, plugin, processor, schema, statusCode, tags }) {
	const result = processor.process({
		export: false,
		meta: {
			resource: "operation",
	...
	const arrowFn = $.func().arrow().$if(result.usesFaker, (f) => f.param("options", (p) => p.optional().type("Options"))).$if(typeSymbol, (f) => f.returns($.type(typeSymbol))).$if(result.usesAccessor, (f) => {
		const fDecl = $.const("f").assign($.binary($("options").attr("faker").optional(), "??", $(plugin.imports.faker)));
		return f.do(fDecl, $.return(result.expression));
	}, (f) => f.do($.return(result.expression)));
	plugin.node($.const(symbol).export().assign(arrowFn));
}
function irOperationToAst$2({ operation, path, plugin, processor, tags }) {
	...
	if (!!(meaningful.length === 1 && meaningful[0].group === "responses")) {
	...
	} else for (const entry of meaningful) exportOperationResponse({
		group: entry.group,
		naming: plugin.config.responses,
		operation,
		path,
...
function irParametersToIrSchema$1(params) {
	...
	if (!Object.keys(properties).length) return void 0;
	return {
		properties,
		required,
		type: "object"
...
*/
function collectRequestParts(operation) {
	const parts = [];
	if (operation.body) parts.push({
		key: "body",
		schema: operation.body.schema
	});
	if (operation.parameters) for (const location of [
		...
		const schema = irParametersToIrSchema$1(params);
...
*/
function irOperationRequestToAst({ operation, path, plugin, processor, tags }) {
	...
	for (const { key, result } of processedParts) obj = obj.prop(key, result.expression);
	...
	const arrowFn = $.func().arrow().$if(usesFaker, (f) => f.param("options", (p) => p.optional().type("Options"))).$if(typeSymbol, (f) => f.returns($.type.expr("Omit").generic($.type(typeSymbol)).generic($.type.literal("url")))).$if(usesAccessor, (f) => {
		const fDecl = $.const("f").assign($.binary($("options").attr("faker").optional(), "??", $(plugin.imports.faker)));
...
//#endregion
//#region src/plugins/@faker-js/faker/shared/export.ts
function exportAst$4({ final, isCircularSchema, meta, naming, namingAnchor, path, plugin, schema, tags }) {
	...
	if (final.resultType === "never") {
		log.warn(`[${plugin.name}] ${symbolIn.name} not generated because we can't produce data that fits its schema.`);
		return;
	}
	...
	const typeSymbol = plugin.querySymbol({
		...
		...meta.role ? { role: meta.role } : void 0
	});
	const arrowFn = $.func().arrow().$if(final.usesFaker, (f) => {
		if (isCircularSchema) return f.param("options", (p) => p.type("Options").assign($.object())).param("_callDepth", (p) => p.assign($.literal(0)));
		return f.param("options", (p) => p.optional().type("Options"));
	}).$if(typeSymbol, (f) => f.returns($.type(typeSymbol))).$if(final.usesAccessor, (f) => {
		const fDecl = $.const("f").assign($.binary($("options").attr("faker").optional(), "??", $(plugin.imports.faker)));
...
//#endregion
//#region src/plugins/@faker-js/faker/v10/processor.ts
function createProcessor$5(plugin, tracking, circularPointers) {
...
//#endregion
//#region src/plugins/@faker-js/faker/v10/plugin.ts
const handlerV10 = ({ plugin }) => {
	const optionsSymbol = plugin.symbol("Options", { kind: "type" });
	const optionsType = $.type.object().prop("faker", (p) => p.optional().type($.type(plugin.imports.Faker))).prop("includeOptional", (p) => p.doc([
		...
		"@default true"
	]).optional().type($.type.or($.type("boolean"), $.type("number")))).prop("useDefault", (p) => p.doc([
		...
		"@default false"
	]).optional().type($.type.or($.type("boolean"), $.type("number"))));
	plugin.node($.type.alias(optionsSymbol).export().type(optionsType));
	const circularPointers = /* @__PURE__ */ new Set();
	const graph = plugin.context.graph;
	if (graph) for (const [pointer, deps] of graph.transitiveDependencies) {
		if (deps.has(pointer)) {
			circularPointers.add(pointer);
			continue;
		}
		for (const dep of deps) if (graph.transitiveDependencies.get(dep)?.has(pointer)) {
			circularPointers.add(pointer);
	...
	const processor = createProcessor$5(plugin, tracking, circularPointers);
	...
	if (tracking.needsMaxCallDepth) {
		...
		plugin.node($.const(maxCallDepthSymbol).assign($.literal(plugin.config.maxCallDepth)), maxCallDepthSlot);
	}
	if (tracking.needsResolveCondition) {
		const conditionParamType = $.type.or($.type("boolean"), $.type("number"));
		const resolveConditionFn = $.func().arrow().param("condition", (p) => p.type(conditionParamType)).param("faker", (p) => p.type($.type(plugin.imports.Faker))).returns("boolean").do($.return($.binary($("condition").eq($.literal(true)), "||", $($.binary($("condition").typeofExpr().eq($.literal("number")), "&&", $("faker").attr("datatype").attr("boolean").call($.object().prop("probability", $("condition"))))))));
...
//#endregion
//#region src/plugins/@faker-js/faker/plugin.ts
const handler$15 = (args) => {
	const { plugin } = args;
	switch (plugin.config.compatibilityVersion) {
		default: return handlerV10(args);
	}
};
//#endregion
//#region src/plugins/@faker-js/faker/config.ts
const defaultConfig$27 = {
	api: new Api$4(),
	config: {
		$cascade: ["case"],
		case: "camelCase",
		compatibilityVersion: coerce((value, context) => {
			...
			function inferCompatibleVersion() {
				if (version && (version.major === 9 || version.major === 10)) return version.major;
				return 10;
			}
	...
	name: "@faker-js/faker",
	symbolMeta() {
		return { artifact: "@faker-js/faker" };
	},
	tags: ["source"]
};
definePluginConfig(defaultConfig$27);
//#endregion
...
function getClientBundlePaths(pluginName) {
	...
	if (isEnvironment("development")) {
		const pluginsDir = path.resolve(__dirname$1, "..", "plugins", "@hey-api");
		return {
	...
	return {
		clientPath: path.resolve(__dirname$1, "clients", clientName),
		corePath: path.resolve(__dirname$1, "clients", "core")
	};
}
/**
...
const createClient = ({ plugin }) => {
	...
	const defaultVals = $.object().$if(baseUrl, (o, v) => o.prop(getClientBaseUrlKey(getTypedConfig(plugin)), $.literal(v))).$if("throwOnError" in plugin.config && plugin.config.throwOnError, (o) => o.prop("throwOnError", $.literal(true)));
...
//#endregion
//#region src/plugins/@hey-api/client-core/create-client-config.ts
...
//#endregion
//#region src/plugins/@hey-api/examples/plugin.ts
const handler$14 = ({ plugin }) => {
	...
	for (const [name, schema] of Object.entries(schemas)) {
		...
		if (hasPluralExamples) generatePluralSchemaFactory({
			functionName,
			name,
			plugin,
			schemaExample
		});
		else generateSingularSchemaFactory({
			functionName,
			name,
			plugin,
			schemaExample
	...
	for (const [, pathItem] of Object.entries(spec.paths)) for (const [, operation] of Object.entries(pathItem)) {
		if ("parameters" in operation || "summary" in operation || "description" in operation) continue;
		if (!("responses" in operation) || !operation.responses) continue;
		const operationExamples = collectOperationExamples(operation.responses, examplesComponent);
		if (operationExamples.statusCodes.size === 0) continue;
...
function resolveSchemaExample(schema, examplesComponent) {
	if (schema.example !== void 0) {
		if (typeof schema.example === "object" && schema.example !== null && "$ref" in schema.example) return resolveRefExample(schema.example.$ref, examplesComponent);
		return schema.example;
	}
	if (schema.examples && Object.keys(schema.examples).length > 0) {
		const resolved = {};
		for (const [key, exampleRef] of Object.entries(schema.examples)) if (typeof exampleRef === "object" && exampleRef !== null && "$ref" in exampleRef) {
			const resolvedValue = resolveRefExample(exampleRef.$ref, examplesComponent);
			if (resolvedValue !== void 0) resolved[key] = resolvedValue;
		} else if (typeof exampleRef === "object" && exampleRef !== null && "value" in exampleRef) resolved[key] = exampleRef.value;
		else resolved[key] = exampleRef;
		return resolved;
	}
	return null;
}
function resolveRefExample($ref, examples) {
	if (!examples) return;
	const refPath = $ref.split("/");
	const example = examples[refPath[refPath.length - 1]];
	if (!example) return;
	return example.value;
}
function getJsonExample(content) {
	if (!content) return null;
	const jsonContent = content["application/json"];
	if (!jsonContent) return null;
	if (jsonContent.example !== void 0) return jsonContent.example;
	if (jsonContent.examples && Object.keys(jsonContent.examples).length > 0) {
		const resolved = {};
		for (const [key, exampleRef] of Object.entries(jsonContent.examples)) if (typeof exampleRef === "object" && exampleRef !== null && "value" in exampleRef) resolved[key] = exampleRef.value;
		else resolved[key] = exampleRef;
		return resolved;
	}
	return null;
}
function collectOperationExamples(responses, examplesComponent) {
	const result = { statusCodes: /* @__PURE__ */ new Map() };
	for (const [statusCode, response] of Object.entries(responses)) {
		const jsonExample = getJsonExample(response.content);
		if (!jsonExample) continue;
		const isPlural = typeof jsonExample === "object" && jsonExample !== null && !Array.isArray(jsonExample);
		let resolvedExamples;
		if (isPlural && typeof jsonExample === "object") {
			const resolved = {};
			for (const [key, value] of Object.entries(jsonExample)) if (typeof value === "object" && value !== null && "$ref" in value) {
				const refResolved = resolveRefExample(value.$ref, examplesComponent);
				if (refResolved !== void 0) resolved[key] = refResolved;
			} else resolved[key] = value;
			resolvedExamples = resolved;
		} else resolvedExamples = { basic: jsonExample };
		if (Object.keys(resolvedExamples).length === 0) continue;
		...
		if (!result.defaultStatusCode) {
			...
			const defaultNum = result.defaultStatusCode ? parseInt(result.defaultStatusCode.replace("X", "0").replace("default", "999")) : 999;
			if (statusCode.startsWith("2") && !result.defaultStatusCode) result.defaultStatusCode = statusCode;
			else if (codeNum < defaultNum && statusCode !== "default") result.defaultStatusCode = statusCode;
		}
	}
	if (!result.defaultStatusCode && result.statusCodes.size > 0) result.defaultStatusCode = Array.from(result.statusCodes.keys())[0];
	return result;
}
function generateSingularSchemaFactory({ functionName, name, plugin, schemaExample }) {
	const symbol = plugin.symbol(functionName, { meta: {
		category: "example",
		resource: "schema",
		resourceId: name
	} });
	const func = $.func(symbol).decl().export().do($.return($.fromValue(schemaExample, { layout: "pretty" })));
	plugin.node(func);
}
function generatePluralSchemaFactory({ functionName, name, plugin, schemaExample }) {
	const firstKey = Object.keys(schemaExample)[0];
	const symbol = plugin.symbol(functionName, { meta: {
		category: "example",
		resource: "schema",
		resourceId: name
	} });
	const func = $.func(symbol).decl().export().do($.return($.fromValue(schemaExample[firstKey], { layout: "pretty" })));
	plugin.node(func);
}
function generateOperationFactory({ examples: operationExamples, functionName, plugin }) {
	...
	defaultStatusCode && parseInt(defaultStatusCode.replace("X", "0"));
	const symbol = plugin.symbol(functionName, { meta: {
		category: "example",
		resource: "operation",
		resourceId: functionName.replace("Example", "")
	} });
	const func = $.func(symbol).decl().export().do($.return($.fromValue(statusCodes[0]?.[1]?.examples.basic ?? statusCodes[0]?.[1]?.examples[Object.keys(statusCodes[0]?.[1]?.examples ?? {})[0]], { layout: "pretty" })));
	plugin.node(func);
}
//#endregion
//#region src/plugins/@hey-api/examples/config.ts
...
//#endregion
//#region src/contracts/schema.ts
var schema_exports = /* @__PURE__ */ __exportAll({
	definition: () => definition$1,
	operation: () => operation
});
function definition$1(resourceId) {
	return {
		category: "schema",
		resource: "definition",
		resourceId
...
const contracts = { schema: schema_exports };
//#endregion
...
//#endregion
//#region src/plugins/@hey-api/sdk/imports.ts
function sdkImports(plugin) {
	const clientModule = clientFolderAbsolutePath(getTypedConfig(plugin));
	const factory = plugin.symbolFactory;
	return {
		...
		ClientMeta: factory.register("ClientMeta", {
			external: clientModule,
			kind: "type",
			meta: { resource: "client.ClientMeta" }
		}),
		Composable: factory.register("Composable", {
			external: clientModule,
			kind: "type",
			meta: { resource: "client.Composable" }
		}),
		...
		RequestResult: factory.register("RequestResult", {
			external: clientModule,
			kind: "type",
			meta: { resource: "client.RequestResult" }
		}),
		ServerSentEventsResult: factory.register("ServerSentEventsResult", {
			external: clientModule,
			kind: "type",
			meta: { resource: "client.ServerSentEventsResult" }
		}),
...
//#endregion
//#region src/plugins/@hey-api/sdk/shared/type-options.ts
function createTypeOptions({ plugin }) {
	...
	const typeOptions = $.type.alias(symbolOptions).export().$if(isNuxtClient, (t) => t.generic("TComposable", (g) => g.extends(plugin.imports.Composable).default($.type.literal("$fetch"))).generic("TData", (g) => g.extends(plugin.imports.TDataShape).default(plugin.imports.TDataShape)).generic(nuxtTypeResponse, (g) => g.default("unknown")).generic(nuxtTypeDefault, (g) => g.default("undefined")), (t) => t.generic("TData", (g) => g.extends(plugin.imports.TDataShape).default(plugin.imports.TDataShape)).generic("ThrowOnError", (g) => g.extends("boolean").default("boolean")).generic("TResponse", (g) => g.default("unknown"))).type($.type.and($.type(plugin.imports.Options).$if(isNuxtClient, (t) => t.generic("TComposable").generic("TData").generic(nuxtTypeResponse).generic(nuxtTypeDefault), (t) => t.generic("TData").generic("ThrowOnError").generic("TResponse")), $.type.object().prop("client", (p) => p.doc([
		"You can provide a client instance returned by `createClient()` instead of",
		"individual options. This might be also useful if you want to implement a",
		"custom client."
	]).required(!plugin.config.client && !isInstance(plugin)).type(plugin.imports.Client)).prop("meta", (p) => p.doc(["You can pass arbitrary values through the `meta` object. This can be", "used to access values that aren't defined as part of the SDK function."]).optional().type($.type.ternary().check($.type.operator().keyof($.type(plugin.imports.ClientMeta))).extends("never").do($.type("Record").generics("string", "unknown")).otherwise($.type(plugin.imports.ClientMeta))))));
...
const handler$12 = (args) => handlerV1$3(args);
//#endregion
...
//#endregion
//#region src/plugins/@hey-api/transformers/imports.ts
function transformersImports(plugin) {
	const factory = plugin.symbolFactory;
	return { temporalPolyfill: TEMPORAL_POLYFILL(factory) };
}
//#endregion
...
function exportEnumAst({ enumData, name, plugin, resourceId, schema }) {
	...
	if (mode === "javascript") {
		...
		const symbolObject = plugin.symbol(buildSymbolIn({
...
function arrayToAst$3({ path, plugin, schema, walk }) {
	...
	return (plugin.config.$resolvers?.array ?? plugin.config["~resolvers"]?.array)?.(ctx) ?? arrayResolver$4(ctx);
}
//#endregion
...
function booleanToAst$3({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.boolean ?? plugin.config["~resolvers"]?.boolean)?.(ctx) ?? booleanResolver$4(ctx);
}
//#endregion
...
function enumToAst$3({ path, plugin, schema }) {
	...
	return {
		enumData,
		type: (plugin.config.$resolvers?.enum ?? plugin.config["~resolvers"]?.enum)?.(ctx) ?? enumResolver$4(ctx)
	};
}
//#endregion
...
function intersectionToAst$3({ childResults, parentSchema, path, plugin, schemas }) {
	...
	return (plugin.config.$resolvers?.intersection ?? plugin.config["~resolvers"]?.intersection)?.(ctx) ?? intersectionResolver$4(ctx);
}
//#endregion
...
function neverToAst$3({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.never ?? plugin.config["~resolvers"]?.never)?.(ctx) ?? neverResolver$4(ctx);
}
//#endregion
...
function nullToAst$4({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.null ?? plugin.config["~resolvers"]?.null)?.(ctx) ?? nullResolver$4(ctx);
}
//#endregion
...
function numberToAst({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.number ?? plugin.config["~resolvers"]?.number)?.(ctx) ?? numberResolver$4(ctx);
}
//#endregion
...
function objectToAst$4({ path, plugin, schema, walk }) {
	...
	return (plugin.config.$resolvers?.object ?? plugin.config["~resolvers"]?.object)?.(ctx) ?? objectResolver$4(ctx);
}
//#endregion
...
function formatNode$4(ctx) {
	...
	if (format === "date-time" || format === "date") {
		...
		if (dates) {
			if (dates === "temporal") return $.type(plugin.imports.temporalPolyfill.Temporal).attr(format === "date" ? "PlainDate" : "Instant");
			return $.type("Date");
		}
	}
...
function stringToAst$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.string ?? plugin.config["~resolvers"]?.string)?.(ctx) ?? stringResolver$4(ctx);
}
//#endregion
...
function tupleToAst$3({ path, plugin, schema, walk }) {
	...
	return (plugin.config.$resolvers?.tuple ?? plugin.config["~resolvers"]?.tuple)?.(ctx) ?? tupleResolver$4(ctx);
}
//#endregion
...
function undefinedToAst$3({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.undefined ?? plugin.config["~resolvers"]?.undefined)?.(ctx) ?? undefinedResolver$4(ctx);
}
//#endregion
...
function baseNode$58(ctx) {
	...
	const discriminatedData = buildDiscriminatedUnion({
		...
		resolveIrRef: (ref) => {
			try {
				return plugin.context.resolveIrRef(ref);
			} catch {
				return;
			}
		},
	...
	if (discriminatedData) {
		...
		for (const member of discriminatedData.members) {
			...
			if (existing) existing.values.push(member.discriminatedValue);
			else grouped.set(member.ref, {
		...
		for (const [ref, group] of grouped) {
			const base = childResults[schemas.findIndex((s) => s.$ref === ref)]?.type;
			if (!base) continue;
			const discriminatorType = group.values.length === 1 ? $.type.literal(group.values[0]) : $.type.or(...group.values.map((v) => $.type.literal(v)));
			members.push($.type.and($.type.object().prop(discriminatedData.discriminatorKey, (p) => p.required(true).type(discriminatorType)), base));
		}
...
function unionToAst$3({ childResults, parentSchema, path, plugin, schemas }) {
	...
	return (plugin.config.$resolvers?.union ?? plugin.config["~resolvers"]?.union)?.(ctx) ?? unionResolver$4(ctx);
}
//#endregion
...
function unknownToAst$3({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.unknown ?? plugin.config["~resolvers"]?.unknown)?.(ctx) ?? unknownResolver$4(ctx);
}
//#endregion
...
function voidToAst$3({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.void ?? plugin.config["~resolvers"]?.void)?.(ctx) ?? voidResolver$4(ctx);
}
//#endregion
...
//#endregion
//#region src/plugins/@hey-api/typescript/imports.ts
function typescriptImports(plugin) {
	const factory = plugin.symbolFactory;
	return { temporalPolyfill: TEMPORAL_POLYFILL(factory) };
}
//#endregion
//#region src/plugins/@hey-api/typescript/shared/client-options.ts
function serverToBaseUrlType({ server }) {
...
function createClientOptions({ nodeIndex, plugin, servers }) {
	const client = getClientPlugin(getTypedConfig(plugin));
...
const operationToType = ({ operation, path, plugin, tags }) => {
	...
	const dataSymbol = plugin.symbol(buildSymbolIn({
		meta: {
			category: "type",
			path,
			resource: "operation",
	...
	if (errors) {
		...
		const errorsSymbol = plugin.symbol(buildSymbolIn({
			meta: {
				category: "type",
				path,
				resource: "operation",
		...
		if (error) {
			const errorSymbol = plugin.symbol(buildSymbolIn({
				meta: {
					category: "type",
					path,
					resource: "operation",
	...
	if (responses) {
		...
		const responsesSymbol = plugin.symbol(buildSymbolIn({
			meta: {
				category: "type",
				path,
				resource: "operation",
		...
		if (response) {
			const responseSymbol = plugin.symbol(buildSymbolIn({
				meta: {
					category: "type",
					path,
					resource: "operation",
...
function webhookToType({ operation, path, plugin, tags }) {
	...
	if (operation.body) {
		symbolWebhookPayload = plugin.symbol(buildSymbolIn({
			meta: {
				category: "type",
				path,
				resource: "webhook",
...
const handler$10 = (args) => handlerV1$2(args);
//#endregion
...
//#endregion
//#region src/plugins/@pinia/colada/imports.ts
function piniaColadaImports(plugin) {
	const factory = plugin.symbolFactory;
	return {
		...PINIA_COLADA(factory),
		axios: AXIOS(factory)
	};
}
//#endregion
...
//#endregion
//#region src/plugins/@pinia/colada/use-type.ts
...
function getPublicTypeData({ isNuxtClient, operation, plugin }) {
...
//#endregion
//#region src/plugins/@pinia/colada/mutation-options.ts
function createMutationOptions$1({ operation, plugin }) {
	const isNuxtClient = getClientPlugin(getTypedConfig(plugin)).name === "@hey-api/client-nuxt";
	...
	const statement = $.const(symbolMutationOptions).export().$if(plugin.config.comments && createOperationComment(operation), (c, v) => c.doc(v)).assign($.func().param(options, (p) => p.optional().type($.type("Partial").generic(typeData))).returns($.type(plugin.imports.UseMutationOptions).generic(useTypeResponse$1({
		operation,
		plugin
...
//#endregion
//#region src/plugins/@pinia/colada/query-key.ts
...
//#endregion
//#region src/plugins/@pinia/colada/query-options.ts
...
const createQueryOptions$1 = ({ operation, plugin }) => {
	...
	const statement = $.const(symbolQueryOptionsFn).export().$if(plugin.config.comments && createOperationComment(operation), (c, v) => c.doc(v)).assign($(plugin.imports.defineQueryOptions).call($.func().param(optionsParamName$2, (p) => p.required(isRequiredOptions).type(typeData)).do($.return(queryOpts))).generics(typeData, useTypeResponse$1({
		operation,
		plugin
	}), useTypeError$1({
		operation,
...
const handler$9 = (args) => handlerV0(args);
//#endregion
...
const defaultConfig$14 = {
	...
	name: "@pinia/colada",
	symbolMeta() {
		return { artifact: "@pinia/colada" };
	}
};
definePluginConfig(defaultConfig$14);
//#endregion
//#region src/plugins/@tanstack/query-core/shared/use-type.ts
...
//#endregion
//#region src/plugins/@tanstack/query-core/v5/get-query-data.ts
...
//#endregion
//#region src/plugins/@tanstack/query-core/query-key.ts
...
//#endregion
//#region src/plugins/@tanstack/query-core/v5/infinite-query-options.ts
...
function createInfiniteQueryOptions({ operation, plugin }) {
	...
	const statement = $.const(symbolInfiniteQueryOptionsFn).export().$if(plugin.config.comments && createOperationComment(operation), (c, v) => c.doc(v)).assign($.func().param("options", (p) => p.required(isRequiredOptions).type(typeData)).do($.const("opts").assign($(plugin.imports.infiniteQueryOptions).call($.object().pretty().hint("@ts-ignore").prop("queryFn", $.func().async().param((p) => p.object("pageParam", "queryKey", "signal")).do(...statements)).prop("queryKey", $(symbolInfiniteQueryKey).call("options")).$if(handleMeta(plugin, operation, "infiniteQueryOptions"), (o, v) => o.prop("meta", v))).generics(typeResponse, useTypeError({
		operation,
		plugin
	}), $.type(plugin.imports.InfiniteData).generic(typeResponse), typeQueryKey, $.type.or(type, typePageObjectParam))), $.return($("opts").as($.type("Omit").generics($("opts").typeofType(), $.type.literal("initialData"))))));
	plugin.node(statement);
}
//#endregion
//#region src/plugins/@tanstack/query-core/mutation-key.ts
...
//#endregion
//#region src/plugins/@tanstack/query-core/v5/mutation-options.ts
...
//#endregion
//#region src/plugins/@tanstack/query-core/v5/query-options.ts
...
function createQueryOptions({ operation, plugin }) {
	...
	const statement = $.const(symbolQueryOptionsFn).export(plugin.config.queryOptions.exported).$if(plugin.config.comments && createOperationComment(operation), (c, v) => c.doc(v)).assign($.func().param(optionsParamName$1, (p) => p.required(isRequiredOptions).type(useTypeData({
		operation,
		plugin
	}))).do($(plugin.imports.queryOptions).call(queryOptionsObj).generics(typeResponse, useTypeError({
		operation,
		plugin
...
//#endregion
//#region src/plugins/@tanstack/query-core/v5/set-query-data.ts
...
//#endregion
//#region src/plugins/@tanstack/query-core/v5/use-get-query-data.ts
...
function createUseMutation({ operation, plugin }) {
	...
	const mutationType = $.type(plugin.imports.MutationOptions).generic(useTypeResponse({
		operation,
		plugin
	})).generic(useTypeError({
		operation,
	...
	func.do($(plugin.imports.useMutation).call($.object().spread($(symbolMutationOptionsFn).call()).spread(mutationOptionsParamName)).return());
...
//#endregion
//#region src/plugins/@tanstack/query-core/v5/use-query.ts
...
function createUseQuery({ operation, plugin }) {
	...
	const statement = $.const(symbolUseQueryFn).export().$if(plugin.config.comments && createOperationComment(operation), (c, v) => c.doc(v)).assign($.func().param(optionsParamName, (p) => p.required(isRequiredOptions).type(typeData)).do($(plugin.imports.useQuery).call($(symbolQueryOptionsFn).call(optionsParamName)).return()));
	plugin.node(statement);
}
//#endregion
//#region src/plugins/@tanstack/query-core/v5/use-set-query-data.ts
...
const handler$8 = (args) => handlerV5(args);
//#endregion
//#region src/plugins/@tanstack/query-core/imports.ts
function tanStackQueryImports(plugin) {
	const factory = plugin.symbolFactory;
	return {
...
const defaultConfig$13 = {
	...
	symbolMeta() {
		return { artifact: "@tanstack/angular-query-experimental" };
	}
};
...
const defaultConfig$12 = {
	...
	symbolMeta() {
		return { artifact: "@tanstack/preact-query" };
	}
};
...
const defaultConfig$11 = {
	...
	symbolMeta() {
		return { artifact: "@tanstack/react-query" };
	}
};
...
const defaultConfig$10 = {
	...
	symbolMeta() {
		return { artifact: "@tanstack/solid-query" };
	}
};
...
const defaultConfig$9 = {
	...
	symbolMeta() {
		return { artifact: "@tanstack/svelte-query" };
	}
};
...
const defaultConfig$8 = {
	...
	symbolMeta() {
		return { artifact: "@tanstack/vue-query" };
	}
};
...
//#endregion
//#region src/plugins/arktype/imports.ts
function arktypeImports(plugin) {
	const factory = plugin.symbolFactory;
	return { ...ARKTYPE(factory) };
}
//#endregion
...
function exportAst$2({ ast, plugin, schema, symbol, typeInferSymbol }) {
	const statement = $.const(symbol).export().$if(plugin.config.comments && createSchemaComment(schema), (c, v) => c.doc(v)).assign($(plugin.imports.type).call(ast.def ? $.literal(ast.def) : ast.expression));
	plugin.node(statement);
	if (typeInferSymbol) {
...
function irSchemaWithTypeToAst({ schema, ...args }) {
	...
	return {
		def: "",
		expression: $(args.plugin.imports.type).call($.object().prop("name", $.literal("string")).prop("platform", $.literal("'android' | 'ios'")).prop("versions?", $.literal("(number | string)[]"))),
...
const handler$7 = (args) => handlerV2$2(args);
//#endregion
...
const defaultConfig$7 = {
	...
	name: "arktype",
	symbolMeta() {
		return { artifact: "arktype" };
	},
	tags: ["validator"]
};
definePluginConfig(defaultConfig$7);
//#endregion
//#region src/plugins/fastify/imports.ts
function fastifyImports(plugin) {
	const factory = plugin.symbolFactory;
	return { ...FASTIFY(factory) };
}
//#endregion
//#region src/plugins/fastify/plugin.ts
function operationToRouteHandler({ operation, plugin }) {
...
const defaultConfig$6 = {
	...
	name: "fastify",
	symbolMeta() {
		return { artifact: "fastify" };
	}
};
definePluginConfig(defaultConfig$6);
//#endregion
//#region src/plugins/msw/imports.ts
function mswImports(plugin) {
	const factory = plugin.symbolFactory;
	return { ...MSW(factory) };
}
//#endregion
//#region src/plugins/msw/shared/compute-dominant-response.ts
function isValidExample(example) {
	if (example === void 0) return false;
	if (example === null) return true;
	const type = typeof example;
	if (type === "string" || type === "number" || type === "boolean") return true;
	if (Array.isArray(example)) return example.every(isValidExample);
	if (type === "object") return Object.values(example).every(isValidExample);
	return false;
}
const KIND_PRIORITY = {
	binary: 1,
...
function computeResponse({ plugin, response, statusCode }) {
	const numericStatus = Number(statusCode);
	if (response.schema.type === "void") return {
		example: void 0,
		kind: "void",
		statusCode: numericStatus
	};
	if (response.schema.type === "unknown" && !response.schema.$ref) return {
		example: void 0,
		kind: "void",
		statusCode: numericStatus
	};
	let schema = response.schema;
	if (schema.$ref) schema = plugin.context.resolveIrRef(schema.$ref);
	const example = isValidExample(schema.example) ? schema.example : void 0;
	...
	return {
		example,
		kind: mediaTypeToKind(response.mediaType),
		statusCode: numericStatus
	};
}
function mediaTypeToKind(mediaType) {
	...
	if (cleanMediaType.startsWith("application/octet-stream") || cleanMediaType.startsWith("audio/") || cleanMediaType.startsWith("image/") || cleanMediaType.startsWith("video/")) return "binary";
	if (cleanMediaType.startsWith("application/json") || cleanMediaType.endsWith("+json")) return "json";
	if (cleanMediaType.startsWith("text/")) return "text";
	return "json";
}
function computeDominantResponse({ operation, plugin }) {
	const candidates = [];
	for (const statusCode in operation.responses) {
		if (statusCodeToGroup({ statusCode }) !== "2XX") continue;
		candidates.push(computeResponse({
			plugin,
	...
	const dominant = candidates.reduce((best, cur) => KIND_PRIORITY[cur.kind] > KIND_PRIORITY[best.kind] ? cur : best);
	return {
...
//#endregion
//#region src/plugins/msw/shared/operation.ts
function getOperationComment(operation) {
	return `Handler for the \`${operation.method.toUpperCase()} ${operation.path}\` operation.`;
}
//#endregion
//#region src/plugins/msw/shared/path.ts
/**
...
function sanitizeParamName(name) {
	return name.replace(/\W+(.)?/g, (_, char) => char ? char.toUpperCase() : "");
}
/**
...
function sanitizePath(path) {
	return path.replace(/\{([^}]+)\}/g, (_, name) => `\0${sanitizeParamName(name)}`).replace(/:/g, String.raw`\:`).replace(/\0/g, ":");
}
//#endregion
//#region src/plugins/msw/shared/response.ts
function createHttpResponse({ plugin, response, symbol }) {
	const init = $.object().prop("status", $(symbol).attr("status").optional().$if(response.statusCode !== void 0, (e) => e.coalesce($.literal(response.statusCode))));
	const body = "body";
	switch (response.kind) {
		case "binary": return $.new(plugin.imports.HttpResponse, body, init);
		case "json": return $(plugin.imports.HttpResponse).attr("json").call(body, init);
		case "text": return $(plugin.imports.HttpResponse).attr("text").call($.ternary($(body).typeofExpr().eq($.literal("string"))).do(body).otherwise($("JSON").attr("stringify").call(body)), init);
		case "void": return $.new(plugin.imports.HttpResponse, body, init);
	}
}
//#endregion
//#region src/plugins/msw/shared/handler.ts
function emitToResponseUnion(plugin) {
	const symbol = plugin.symbol("ToResponseUnion", { meta: {
		category: "type",
		resource: "to-response-union"
	} });
	const extractKeyofTNumber = $.type("Extract").generic($.type("keyof T")).generic($.type("number"));
	const toResponseUnionType = $.type.alias(symbol).generic("T").type($.type.idx($.type.mapped("K").key(extractKeyofTNumber).type($.type.object().prop("status", (p) => p.type("K")).prop("body", (p) => p.type($.type("T").idx($.type("K"))))), $.type("Extract").generic($.type("keyof T")).generic($.type("number"))));
	plugin.node(toResponseUnionType);
}
function createHandlerNode({ baseUrl, bodyType, operation, paramsType, plugin, response, responseParamType }) {
	const symbolHttp = plugin.imports.http;
	const symbolResponse = plugin.symbol("response");
	const symbolOptions = plugin.symbol("options");
	const symbol = plugin.symbol(applyNaming(`handle-${operation.id}`, { casing: "camelCase" }));
	const notImplementedResponse = $.new("Response", $.literal("Not Implemented"), $.object().pretty().prop("status", $.literal(501)).prop("statusText", $.literal("Not Implemented")));
	const hasResponse = response.example !== void 0;
	const handlerFunc = $.func(symbol).export().$if(plugin.config.comments && getOperationComment(operation), (f, v) => f.doc(v)).param(symbolResponse, (p) => p.optional().type(responseParamType)).param(symbolOptions, (p) => p.optional().type(plugin.referenceSymbol({
		artifact: "msw",
		category: "type",
		resource: "request-handler-options"
	}))).returns(plugin.imports.HttpHandler).do($(symbolHttp).attr(operation.method).call($.template($(symbolOptions).attr("baseUrl").optional().coalesce($.literal(baseUrl ?? ""))).add(sanitizePath(operation.path)), $.func().param("info").do($.if($.typeofExpr(symbolResponse).eq($.literal("function"))).do($(symbolResponse).call("info").return()), $.const("body").assign($(symbolResponse).attr("body").optional().$if(hasResponse, (c) => c.coalesce($.fromValue(response.example)))), $.if($("body").neq($("undefined"))).do(createHttpResponse({
		plugin,
		response,
		symbol: symbolResponse
	}).return())).$if(!hasResponse, (f) => f.$if(plugin.config.responseFallback === "error", (f) => f.do($.if($(symbolOptions).attr("responseFallback").optional().eq($.literal("passthrough"))).do($.return()), notImplementedResponse.return()), (f) => f.do($.if($(symbolOptions).attr("responseFallback").optional().eq($.literal("error"))).do(notImplementedResponse.return())))), symbolOptions).generics(paramsType, bodyType).return());
	plugin.node(handlerFunc);
	return symbol;
}
function getHandler({ baseUrl, operation, plugin }) {
	const response = computeDominantResponse({
		operation,
		plugin
	});
	...
	let bodyType;
	if (operation.body && symbolDataType) if (operation.body?.schema.type === "unknown") bodyType = $.type(plugin.imports.DefaultBodyType);
	else bodyType = $.type(symbolDataType).idx($.type.literal("body"));
	else bodyType = $.type("never");
	let paramsType;
	if (operation.parameters?.path && Object.keys(operation.parameters.path).length) {
		const objType = $.type.object();
		for (const parameter of Object.values(operation.parameters.path ?? {})) objType.prop(sanitizeParamName(parameter.name), (p) => p.type("string"));
		paramsType = objType;
	} else paramsType = $.type("never");
	const resolverType = $.type(plugin.imports.HttpResponseResolver).generics(paramsType, bodyType);
	if (!plugin.config.source.includes("@hey-api/examples")) response.example = void 0;
	const responseType = $.type.object().prop("body", (p) => p.type(response.statusCode !== void 0 && symbolResponsesType ? $.type(symbolResponsesType).idx($.type.literal(response.statusCode)) : $.type(plugin.imports.DefaultBodyType))).prop("status", (p) => p.optional().type(response.statusCode !== void 0 ? $.type.literal(response.statusCode) : $.type("number")));
	const symbolResponseType = plugin.symbol(applyNaming(`handle-${operation.id}-response`, { casing: "PascalCase" }));
	plugin.node($.type.alias(symbolResponseType).export().type(responseType));
	let responsesOverrideType;
	if (symbolResponsesType && response.allCandidates.length > 1) {
		if (!plugin.querySymbol({
			category: "type",
			resource: "to-response-union"
		})) emitToResponseUnion(plugin);
		responsesOverrideType = $.type(plugin.referenceSymbol({
			category: "type",
			resource: "to-response-union"
		})).generic(symbolResponsesType);
	}
	const responseParamType = $.type.or(symbolResponseType, responsesOverrideType ? $.type.or(responsesOverrideType, resolverType) : resolverType);
...
//#endregion
//#region src/plugins/msw/shared/sort.ts
/**
...
function sortHandlers(handlers) {
	return handlers.map((info, index) => ({
		index,
		info
	})).sort((a, b) => {
...
//#endregion
//#region src/plugins/msw/shared/types.ts
function createRequestHandlerOptions(plugin) {
	const symbol = plugin.symbol("RequestHandlerOptions", { meta: {
		artifact: "msw",
		category: "type",
		resource: "request-handler-options"
	} });
	...
	const node = $.type.alias(symbol).export().type($.type.and($.type(plugin.imports.RequestHandlerOptions), $.type.object().prop("baseUrl", (p) => p.$if(symbolBaseUrl, (p, s) => p.type($.type(s).idx($.type.literal(getClientBaseUrlKey(getTypedConfig(plugin))))), (p) => p.type("string")).optional()).prop("responseFallback", (p) => p.type($.type.or($.type.literal("error"), $.type.literal("passthrough"))).optional())));
	plugin.node(node);
	return symbol;
}
//#endregion
//#region src/plugins/msw/v2/plugin.ts
const handlerV2$1 = ({ plugin }) => {
	...
	plugin.forEach("operation", ({ operation }) => {
		...
		pickType.prop(name, (p) => p.type($(symbolHandler).typeofType()).$if(plugin.config.comments && getOperationComment(operation), (f, v) => f.doc(v)));
	...
	const factoryReturnType = $.type.alias(symbolFactoryReturnType).export().type($.type.object().prop(factoryResultAll, (p) => p.type($.type.func().param("options", (p) => p.optional().type($.type.object().prop(factoryResultPick, (p) => p.optional().type($.type.mapped("K").key($.type.operator().keyof($.type(symbolHandlerFactoriesType))).optional().type($.type.or($.type("Parameters").generic($.type(symbolHandlerFactoriesType).idx("K")).idx($.type.literal(0)), $.type("Parameters").generic($.type(symbolHandlerFactoriesType).idx("K")))))))).returns($.type("ReadonlyArray").generic(plugin.imports.HttpHandler)))).prop(factoryResultPick, (p) => p.type(symbolHandlerFactoriesType)));
	plugin.node(factoryReturnType);
	const factoryFn = $.func(symbolFactory).export().param("config", (p) => p.type(symbolRequestHandlerOptions).assign($.object())).returns(symbolFactoryReturnType).do($.type.alias(symbolHandler).generic("R").type($.type.func().param("response", (p) => p.optional().type("R")).param("options", (p) => p.optional().type(symbolRequestHandlerOptions)).returns(plugin.imports.HttpHandler)), $.func(symbolWrap).generic("R").param("handler", (p) => p.type($.type(symbolHandler).generic("R"))).returns($.type(symbolHandler).generic("R")).do($.return($.func().param("response").param("options").do($.return($("handler").call("response", $.object().spread("config").spread("options")))))), $.const(symbolPick).type($.type(symbolFactoryReturnType).idx($.type.literal(factoryResultPick))).assign(pickObject), $.const(symbolAll).type($.type(symbolFactoryReturnType).idx($.type.literal(factoryResultAll))).assign($.func().param("options", (p) => p.assign($.object())).do($.type.alias(symbolOverrideValue).generic("R").type($.type.or($.type("R"), $.type.tuple($.type.tupleMember("response").optional().type("R"), $.type.tupleMember("options").optional().type(symbolRequestHandlerOptions)))), $.func(symbolInvoke).generic("R").param("fn", (p) => p.type($.type(symbolHandler).generic("R"))).param("override", (p) => p.optional().type($.type(symbolOverrideValue).generic("R"))).returns(plugin.imports.HttpHandler).do($.return($.ternary($("Array").attr("isArray").call("override")).do($("fn").call($.spread("override"))).otherwise($("fn").call("override")))), $.const("overrides").assign($("options").attr(factoryResultPick).coalesce($.object())), $.array(...sortHandlers(handlerInfo).map((info) => $(symbolInvoke).call($(symbolPick).attr(info.name), $("overrides").attr(info.name)))).return())), $.object().prop(factoryResultAll, symbolAll).prop(factoryResultPick, symbolPick).return());
	plugin.node(factoryFn);
};
//#endregion
//#region src/plugins/msw/plugin.ts
const handler$5 = (args) => handlerV2$1(args);
//#endregion
//#region src/plugins/msw/config.ts
const defaultConfig$5 = {
	config: {
		...
		source: coerce((value) => {
			if (!value) return ["@hey-api/examples"];
...
const defaultConfig$4 = {
	...
	name: "nestjs",
	symbolMeta() {
		return { artifact: "nestjs" };
	}
};
definePluginConfig(defaultConfig$4);
//#endregion
//#region src/plugins/orpc/imports.ts
function orpcImports(plugin) {
	const factory = plugin.symbolFactory;
	return { contract: ORPC_CONTRACT(factory) };
}
//#endregion
...
const defaultConfig$3 = {
	...
	name: "orpc",
	symbolMeta() {
		return { artifact: "orpc" };
	}
};
definePluginConfig(defaultConfig$3);
//#endregion
//#region src/plugins/swr/imports.ts
function swrImports(plugin) {
	const factory = plugin.symbolFactory;
	return { ...SWR(factory) };
}
//#endregion
//#region src/plugins/swr/v2/use-swr.ts
function createUseSwr({ operation, plugin }) {
	if (hasOperationSse({ operation })) return;
	...
	const statement = $.const(symbolUseQueryFn).export().$if(plugin.config.comments && createOperationComment(operation), (c, v) => c.doc(v)).assign($.func().do($(plugin.imports.useSWR).call($.literal(operation.path), $.func().async().do(...statements)).return()));
	plugin.node(statement);
}
//#endregion
...
function exportAst$1({ final, meta, naming, namingAnchor, path, plugin, schema, tags }) {
	...
	const statement = $.const(symbol).export().$if(plugin.config.comments && createSchemaComment(schema), (c, v) => c.doc(v)).$if(final.typeName, (c) => c.type($.type(plugin.imports.v).attr(final.typeName))).assign(pipesToNode(final.pipes, plugin));
	plugin.node(statement);
	return symbol;
}
//#endregion
...
function compositeNode$3(ctx) {
	...
	for (const key of requestValidatorLayers) {
		...
		if (layerSchema) {
			...
			if (!layer.optional) allOptional = false;
			continue;
		}
	...
	let result = $(v).attr(identifiers$1.schemas.object).call(obj);
	if (allOptional && ctx.outerOptional) result = $(v).attr(identifiers$1.schemas.optional).call(result);
	return result;
}
...
function runRequestResolver$3(ctx) {
	const validator = ctx.plugin.config.$resolvers?.validator ?? ctx.plugin.config["~resolvers"]?.validator;
...
//#endregion
//#region src/plugins/valibot/imports.ts
function valibotImports(plugin) {
	const factory = plugin.symbolFactory;
...
function unknownToPipes({ path, plugin, schema }) {
	const resolverCtx = {
		...
		symbols: { v: plugin.imports.v }
	};
	const node = (plugin.config.$resolvers?.unknown ?? plugin.config["~resolvers"]?.unknown)?.(resolverCtx) ?? unknownResolver$3(resolverCtx);
...
function arrayToPipes(ctx) {
	...
	const node = (plugin.config.$resolvers?.array ?? plugin.config["~resolvers"]?.array)?.(resolverCtx) ?? arrayResolver$3(resolverCtx);
	return {
		childResults,
...
function booleanToPipes({ path, plugin, schema }) {
	...
	const node = (plugin.config.$resolvers?.boolean ?? plugin.config["~resolvers"]?.boolean)?.(resolverCtx) ?? booleanResolver$3(resolverCtx);
	return resolverCtx.pipes.toNode(node, plugin);
}
//#endregion
...
function enumToPipes({ path, plugin, schema }) {
	...
	const node = (plugin.config.$resolvers?.enum ?? plugin.config["~resolvers"]?.enum)?.(resolverCtx) ?? enumResolver$3(resolverCtx);
	return resolverCtx.pipes.toNode(node, plugin);
}
//#endregion
...
function intersectionToPipes({ applyModifiers, childResults, parentSchema, path, plugin }) {
	...
	const node = (plugin.config.$resolvers?.intersection ?? plugin.config["~resolvers"]?.intersection)?.(resolverCtx) ?? intersectionResolver$3(resolverCtx);
	return {
		childResults,
		pipes: [resolverCtx.pipes.toNode(node, plugin)]
	};
...
function neverToPipes({ path, plugin, schema }) {
	...
	const node = (plugin.config.$resolvers?.never ?? plugin.config["~resolvers"]?.never)?.(resolverCtx) ?? neverResolver$3(resolverCtx);
	return resolverCtx.pipes.toNode(node, plugin);
}
//#endregion
...
function nullToPipes({ path, plugin, schema }) {
	...
	const node = (plugin.config.$resolvers?.null ?? plugin.config["~resolvers"]?.null)?.(resolverCtx) ?? nullResolver$3(resolverCtx);
	return resolverCtx.pipes.toNode(node, plugin);
}
//#endregion
...
function numberToPipes({ path, plugin, schema }) {
	...
	const node = (plugin.config.$resolvers?.number ?? plugin.config["~resolvers"]?.number)?.(resolverCtx) ?? numberResolver$3(resolverCtx);
	return resolverCtx.pipes.toNode(node, plugin);
}
//#endregion
...
function objectToPipes(ctx) {
	...
	const node = (plugin.config.$resolvers?.object ?? plugin.config["~resolvers"]?.object)?.(resolverCtx) ?? objectResolver$3(resolverCtx);
	return {
		childResults,
		pipes: [resolverCtx.pipes.toNode(node, plugin)]
	};
...
//#endregion
//#region src/plugins/valibot/shared/extensions.ts
function getPatternMessage$1(schema) {
...
function patternNode$3(ctx) {
	...
	const message = getPatternMessage$1(schema);
	return $(v).attr(identifiers$1.actions.regex).call($.regexp(schema.pattern, flags), ...message ? [$.literal(message)] : []);
}
...
function stringToPipes({ path, plugin, schema }) {
	...
	const node = (plugin.config.$resolvers?.string ?? plugin.config["~resolvers"]?.string)?.(resolverCtx) ?? stringResolver$3(resolverCtx);
	return resolverCtx.pipes.toNode(node, plugin);
}
//#endregion
...
function tupleToPipes(ctx) {
	...
	const node = (plugin.config.$resolvers?.tuple ?? plugin.config["~resolvers"]?.tuple)?.(resolverCtx) ?? tupleResolver$3(resolverCtx);
	return {
		childResults,
		pipes: [resolverCtx.pipes.toNode(node, plugin)]
	};
...
function undefinedToPipes({ path, plugin, schema }) {
	...
	const node = (plugin.config.$resolvers?.undefined ?? plugin.config["~resolvers"]?.undefined)?.(resolverCtx) ?? undefinedResolver$3(resolverCtx);
	return resolverCtx.pipes.toNode(node, plugin);
}
//#endregion
...
function baseNode$43(ctx) {
	...
	if (discriminatedData) {
		if (discriminatedData.members.some((m) => m.needsExtend)) {
			const refToNode = /* @__PURE__ */ new Map();
			for (let i = 0; i < schemas.length; i++) {
				const schema = schemas[i];
				if (schema.type !== "null" && schema.const !== null && schema.$ref) refToNode.set(schema.$ref, pipesToNode(childResults[i].pipes, plugin));
			}
			const memberNodes = discriminatedData.members.map((member) => {
				const childNode = refToNode.get(member.ref);
				if (member.needsExtend) return $(v).attr(identifiers$1.schemas.intersect).call($.array($(v).attr(identifiers$1.schemas.object).call($.object().pretty().prop(discriminatedData.discriminatorKey, $(v).attr(identifiers$1.schemas.literal).call($.fromValue(member.discriminatedValue)))), childNode));
				return childNode;
			});
			return $(v).attr(identifiers$1.schemas.union).call($.array(...memberNodes));
		}
		const itemNodes = nonNullItems.map((i) => pipesToNode(i.pipes, plugin));
		return $(v).attr(identifiers$1.schemas.variant).call($.literal(discriminatedData.discriminatorKey), $.array(...itemNodes));
	}
	const itemNodes = nonNullItems.map((i) => pipesToNode(i.pipes, plugin));
...
function unionToPipes({ applyModifiers, childResults, parentSchema, path, plugin, schemas }) {
	...
	const node = (plugin.config.$resolvers?.union ?? plugin.config["~resolvers"]?.union)?.(resolverCtx) ?? unionResolver$3(resolverCtx);
	return {
		childResults,
		pipes: [resolverCtx.pipes.toNode(node, plugin)]
	};
...
function voidToPipes({ path, plugin, schema }) {
	...
	const node = (plugin.config.$resolvers?.void ?? plugin.config["~resolvers"]?.void)?.(resolverCtx) ?? voidResolver$3(resolverCtx);
	return resolverCtx.pipes.toNode(node, plugin);
}
//#endregion
...
//#endregion
//#region src/plugins/zod/contracts.ts
function definition(resourceId) {
	return {
		artifact: "zod",
		category: "schema",
		resource: "definition",
...
function operationRequest(resourceId, layer) {
	return {
		artifact: "zod",
		category: "schema",
		resource: "operation",
...
function operationResponses(resourceId) {
	return {
		artifact: "zod",
		category: "schema",
		resource: "operation",
...
function exportAst({ final, meta, naming, namingAnchor, path, plugin, schema, tags }) {
	...
	if (naming.types.infer.enabled) {
		const typeInferSymbol = plugin.symbol(buildSymbolIn({
			meta: {
				category: "type",
				path,
				tags,
	...
	if (naming.types.input.enabled) {
		const typeInputSymbol = plugin.symbol(buildSymbolIn({
			meta: {
				category: "type",
				path,
				tags,
	...
	if (naming.types.output.enabled) {
		const typeOutputSymbol = plugin.symbol(buildSymbolIn({
			meta: {
				category: "type",
				path,
				tags,
...
function compositeNode$2(ctx) {
	...
	for (const key of requestValidatorLayers) {
		const layer = resolveValidatorLayer(ctx.layers, key, defaultValues);
		const layerSchema = ctx.plugin.querySymbol(ZodContracts.operationRequest(ctx.operation.id, key));
		if (layerSchema) {
			obj.prop(layer.as, ctx.nodes.optional({
				...ctx,
				layer,
	...
	if (obj.isEmpty) return;
	let result = $(z).attr(identifiers.object).call(obj);
	if (allOptional && ctx.outerOptional) result = $(z).attr(identifiers.optional).call(result);
	return result;
}
...
function compositeNode$1(ctx) {
	...
	if (obj.isEmpty) return;
	let result = $(z).attr(identifiers.object).call(obj);
	if (allOptional && ctx.outerOptional) result = $(result).attr(identifiers.optional).call();
	return result;
}
...
//#endregion
//#region src/plugins/zod/imports.ts
function zodImports(plugin) {
	return { ...ZOD(plugin) };
}
//#endregion
...
function unknownToAst$2({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.unknown ?? plugin.config["~resolvers"]?.unknown)?.(ctx) ?? unknownResolver$2(ctx);
}
//#endregion
...
function arrayToAst$2({ applyModifiers, path, plugin, schema, walk }) {
	...
	return {
		chain: (plugin.config.$resolvers?.array ?? plugin.config["~resolvers"]?.array)?.(ctx) ?? arrayResolver$2(ctx),
		childResults
	};
}
//#endregion
...
function booleanToAst$2({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.boolean ?? plugin.config["~resolvers"]?.boolean)?.(ctx) ?? booleanResolver$2(ctx);
}
//#endregion
...
function enumToAst$2({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.enum ?? plugin.config["~resolvers"]?.enum)?.(ctx) ?? enumResolver$2(ctx);
}
//#endregion
...
function intersectionToAst$2({ childResults, parentSchema, path, plugin, schemas }) {
	...
	return {
		chain: (plugin.config.$resolvers?.intersection ?? plugin.config["~resolvers"]?.intersection)?.(ctx) ?? intersectionResolver$2(ctx),
		childResults
	};
}
//#endregion
...
function neverToAst$2({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.never ?? plugin.config["~resolvers"]?.never)?.(ctx) ?? neverResolver$2(ctx);
}
//#endregion
...
function nullToAst$2({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.null ?? plugin.config["~resolvers"]?.null)?.(ctx) ?? nullResolver$2(ctx);
}
//#endregion
...
function numberToNode$2({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.number ?? plugin.config["~resolvers"]?.number)?.(ctx) ?? numberResolver$2(ctx);
}
//#endregion
...
function objectToAst$2(options) {
	...
	return {
		chain: (plugin.config.$resolvers?.object ?? plugin.config["~resolvers"]?.object)?.(ctx) ?? objectResolver$2(ctx),
		childResults
	};
}
//#endregion
//#region src/plugins/zod/shared/extensions.ts
function getPatternMessage(schema) {
	const message = schema["x-pattern-message"];
	if (typeof message !== "string") return;
	return message;
}
...
function patternNode$2(ctx) {
	...
	return $(z).attr(identifiers.regex).call($.regexp(schema.pattern, flags), ...message ? [$.literal(message)] : []);
}
...
function stringToNode$2({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.string ?? plugin.config["~resolvers"]?.string)?.(ctx) ?? stringResolver$2(ctx);
}
//#endregion
...
function tupleToAst$2({ applyModifiers, path, plugin, schema, walk }) {
	...
	return {
		chain: (plugin.config.$resolvers?.tuple ?? plugin.config["~resolvers"]?.tuple)?.(ctx) ?? tupleResolver$2(ctx),
		childResults
	};
}
//#endregion
...
function undefinedToAst$2({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.undefined ?? plugin.config["~resolvers"]?.undefined)?.(ctx) ?? undefinedResolver$2(ctx);
}
//#endregion
//#region src/plugins/zod/shared/discriminator.ts
...
function shouldFallBackToUnion({ childResults, parentSchema, plugin, schemas }) {
	...
	for (let index = 0; index < schemas.length; index++) {
		...
		let resolved;
		try {
			resolved = plugin.context.resolveIrRef(ref);
		} catch {
			continue;
		}
		if ((plugin.querySymbol(ZodContracts.definition(ref))?.meta)?.isIntersection) {
			if (!(resolved?.logicalOperator === "and" && resolved.items?.length === 1)) return true;
		}
	...
	if (discriminatedData) {
		if (!shouldFallBackToUnion({
			...
			const unionMembers = discriminatedData.members.map((member) => {
				...
				return member.needsExtend ? $(z).attr(identifiers.extend).call(refExpr, $.object().prop(discriminatedData.discriminatorKey, $(z).attr(identifiers.literal).call($.fromValue(member.discriminatedValue)))) : refExpr;
			});
			return $(z).attr(identifiers.discriminatedUnion).call($.literal(discriminatedData.discriminatorKey), $.array().pretty().elements(...unionMembers));
		}
		const refToChildChain = /* @__PURE__ */ new Map();
		for (let i = 0; i < schemas.length; i++) {
			const schema = schemas[i];
			if (schema.type !== "null" && schema.const !== null && schema.$ref) refToChildChain.set(schema.$ref, childResults[i].chain);
		}
		const unionMembers = discriminatedData.members.map((member) => {
			const childChain = refToChildChain.get(member.ref);
			return member.needsExtend ? $(z).attr(identifiers.intersection).call($(z).attr(identifiers.object).call($.object().pretty().prop(discriminatedData.discriminatorKey, $(z).attr(identifiers.literal).call($.fromValue(member.discriminatedValue)))), childChain) : childChain;
		});
		return $(z).attr(identifiers.union).call($.array().pretty().elements(...unionMembers));
	}
...
function unionToAst$2({ childResults, parentSchema, path, plugin, schemas }) {
	...
	return {
		chain: (plugin.config.$resolvers?.union ?? plugin.config["~resolvers"]?.union)?.(ctx) ?? unionResolver$2(ctx),
		childResults
	};
}
//#endregion
...
function voidToAst$2({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.void ?? plugin.config["~resolvers"]?.void)?.(ctx) ?? voidResolver$2(ctx);
}
//#endregion
...
function unknownToAst$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.unknown ?? plugin.config["~resolvers"]?.unknown)?.(ctx) ?? unknownResolver$1(ctx);
}
//#endregion
...
function arrayToAst$1({ applyModifiers, path, plugin, schema, walk }) {
	...
	return {
		chain: (plugin.config.$resolvers?.array ?? plugin.config["~resolvers"]?.array)?.(ctx) ?? arrayResolver$1(ctx),
		childResults
	};
}
//#endregion
...
function booleanToAst$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.boolean ?? plugin.config["~resolvers"]?.boolean)?.(ctx) ?? booleanResolver$1(ctx);
}
//#endregion
...
function enumToAst$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.enum ?? plugin.config["~resolvers"]?.enum)?.(ctx) ?? enumResolver$1(ctx);
}
//#endregion
...
function intersectionToAst$1({ childResults, parentSchema, path, plugin, schemas }) {
	...
	return {
		chain: (plugin.config.$resolvers?.intersection ?? plugin.config["~resolvers"]?.intersection)?.(ctx) ?? intersectionResolver$1(ctx),
		childResults
	};
}
//#endregion
...
function neverToAst$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.never ?? plugin.config["~resolvers"]?.never)?.(ctx) ?? neverResolver$1(ctx);
}
//#endregion
...
function nullToAst$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.null ?? plugin.config["~resolvers"]?.null)?.(ctx) ?? nullResolver$1(ctx);
}
//#endregion
...
function numberToNode$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.number ?? plugin.config["~resolvers"]?.number)?.(ctx) ?? numberResolver$1(ctx);
}
//#endregion
...
function objectToAst$1(options) {
	...
	return {
		chain: (plugin.config.$resolvers?.object ?? plugin.config["~resolvers"]?.object)?.(ctx) ?? objectResolver$1(ctx),
		childResults
	};
}
//#endregion
...
function patternNode$1(ctx) {
	...
	const flags = /\\[pP]\{/.test(schema.pattern) ? "u" : void 0;
	const message = getPatternMessage(schema);
	return chain.current.attr(identifiers.regex).call($.regexp(schema.pattern, flags), ...message ? [$.literal(message)] : []);
}
...
function stringToNode$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.string ?? plugin.config["~resolvers"]?.string)?.(ctx) ?? stringResolver$1(ctx);
}
//#endregion
...
function tupleToAst$1({ applyModifiers, path, plugin, schema, walk }) {
	...
	return {
		chain: (plugin.config.$resolvers?.tuple ?? plugin.config["~resolvers"]?.tuple)?.(ctx) ?? tupleResolver$1(ctx),
		childResults
	};
}
//#endregion
...
function undefinedToAst$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.undefined ?? plugin.config["~resolvers"]?.undefined)?.(ctx) ?? undefinedResolver$1(ctx);
}
//#endregion
...
function baseNode$15(ctx) {
	...
	if (discriminatedData) {
		if (!shouldFallBackToUnion({
			...
			const unionMembers = discriminatedData.members.map((member) => {
				const refExpr = $(plugin.referenceSymbol(ZodContracts.definition(member.ref)));
				return member.needsExtend ? refExpr.attr(identifiers.extend).call($.object().prop(discriminatedData.discriminatorKey, $(z).attr(identifiers.literal).call($.fromValue(member.discriminatedValue)))) : refExpr;
			});
			return $(z).attr(identifiers.discriminatedUnion).call($.literal(discriminatedData.discriminatorKey), $.array().pretty().elements(...unionMembers));
		}
		const refToChildChain = /* @__PURE__ */ new Map();
		...
		const unionMembers = discriminatedData.members.map((member) => {
			const childChain = refToChildChain.get(member.ref);
			return member.needsExtend ? $(z).attr(identifiers.object).call($.object().pretty().prop(discriminatedData.discriminatorKey, $(z).attr(identifiers.literal).call($.fromValue(member.discriminatedValue)))).attr(identifiers.and).call(childChain) : childChain;
		});
		return $(z).attr(identifiers.union).call($.array().pretty().elements(...unionMembers));
	}
	return $(z).attr(identifiers.union).call($.array().pretty().elements(...nonNullItems.map((item) => item.chain)));
...
function unionToAst$1({ childResults, parentSchema, path, plugin, schemas }) {
	...
	return {
		chain: (plugin.config.$resolvers?.union ?? plugin.config["~resolvers"]?.union)?.(ctx) ?? unionResolver$1(ctx),
		childResults
	};
}
//#endregion
...
function voidToAst$1({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.void ?? plugin.config["~resolvers"]?.void)?.(ctx) ?? voidResolver$1(ctx);
}
//#endregion
...
function unknownToAst({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.unknown ?? plugin.config["~resolvers"]?.unknown)?.(ctx) ?? unknownResolver(ctx);
}
//#endregion
...
function arrayToAst({ applyModifiers, path, plugin, schema, walk }) {
	...
	return {
		chain: (plugin.config.$resolvers?.array ?? plugin.config["~resolvers"]?.array)?.(ctx) ?? arrayResolver(ctx),
		childResults
	};
}
//#endregion
...
function booleanToAst({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.boolean ?? plugin.config["~resolvers"]?.boolean)?.(ctx) ?? booleanResolver(ctx);
}
//#endregion
...
function enumToAst({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.enum ?? plugin.config["~resolvers"]?.enum)?.(ctx) ?? enumResolver(ctx);
}
//#endregion
...
function intersectionToAst({ childResults, parentSchema, path, plugin, schemas }) {
	...
	return {
		chain: (plugin.config.$resolvers?.intersection ?? plugin.config["~resolvers"]?.intersection)?.(ctx) ?? intersectionResolver(ctx),
		childResults
	};
}
//#endregion
...
function neverToAst({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.never ?? plugin.config["~resolvers"]?.never)?.(ctx) ?? neverResolver(ctx);
}
//#endregion
...
function nullToAst({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.null ?? plugin.config["~resolvers"]?.null)?.(ctx) ?? nullResolver(ctx);
}
//#endregion
...
function numberToNode({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.number ?? plugin.config["~resolvers"]?.number)?.(ctx) ?? numberResolver(ctx);
}
//#endregion
...
function objectToAst({ applyModifiers, path, plugin, schema, walk }) {
	...
	return {
		chain: (plugin.config.$resolvers?.object ?? plugin.config["~resolvers"]?.object)?.(ctx) ?? objectResolver(ctx),
		childResults
	};
}
//#endregion
...
function stringToNode({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.string ?? plugin.config["~resolvers"]?.string)?.(ctx) ?? stringResolver(ctx);
}
//#endregion
...
function tupleToAst({ applyModifiers, path, plugin, schema, walk }) {
	...
	return {
		chain: (plugin.config.$resolvers?.tuple ?? plugin.config["~resolvers"]?.tuple)?.(ctx) ?? tupleResolver(ctx),
		childResults
	};
}
//#endregion
...
function undefinedToAst({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.undefined ?? plugin.config["~resolvers"]?.undefined)?.(ctx) ?? undefinedResolver(ctx);
}
//#endregion
...
function unionToAst({ childResults, parentSchema, path, plugin, schemas }) {
	...
	return {
		chain: (plugin.config.$resolvers?.union ?? plugin.config["~resolvers"]?.union)?.(ctx) ?? unionResolver(ctx),
		childResults
	};
}
//#endregion
...
function voidToAst({ path, plugin, schema }) {
	...
	return (plugin.config.$resolvers?.void ?? plugin.config["~resolvers"]?.void)?.(ctx) ?? voidResolver(ctx);
}
//#endregion
...
const defaultPluginConfigs = {
	"@angular/common": defaultConfig$28,
	"@faker-js/faker": defaultConfig$27,
	"@hey-api/client-angular": defaultConfig$26,
	"@hey-api/client-axios": defaultConfig$25,
	"@hey-api/client-fetch": defaultConfig$24,
	"@hey-api/client-ky": defaultConfig$23,
	"@hey-api/client-next": defaultConfig$22,
	"@hey-api/client-nuxt": defaultConfig$21,
	"@hey-api/client-ofetch": defaultConfig$20,
	"@hey-api/examples": defaultConfig$19,
	"@hey-api/schemas": defaultConfig$18,
	"@hey-api/sdk": defaultConfig$17,
	"@hey-api/transformers": defaultConfig$16,
	"@hey-api/typescript": defaultConfig$15,
	"@pinia/colada": defaultConfig$14,
	"@tanstack/angular-query-experimental": defaultConfig$13,
	"@tanstack/preact-query": defaultConfig$12,
	"@tanstack/react-query": defaultConfig$11,
	"@tanstack/solid-query": defaultConfig$10,
	"@tanstack/svelte-query": defaultConfig$9,
	"@tanstack/vue-query": defaultConfig$8,
	arktype: defaultConfig$7,
	fastify: defaultConfig$6,
	msw: defaultConfig$5,
...
//#endregion
export { getTypedConfig as _, generateClientBundle as a, getClientPlugin as c, TsDslContext as d, ctx as f, TsDsl as g, regexp as h, clientPluginHandler as i, $ as l, keywords as m, postProcessors as n, clientDefaultConfig as o, reserved as p, defaultPlugins as r, clientDefaultMeta as s, resolveJobs as t, TypeScriptRenderer as u };

//# sourceMappingURL=init-D6Y8JFUS.mjs.map