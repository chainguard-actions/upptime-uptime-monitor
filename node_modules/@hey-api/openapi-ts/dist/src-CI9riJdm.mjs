...
async function createClient$1({ config, dependencies, jobIndex, logger, watches: _watches }) {
	...
	if (!_watches) logInputPaths(inputPaths, jobIndex, config.logs.level);
	...
	if (specData.length) {
		...
		if (!config.dryRun) {
			...
			if (config.logs.level !== "silent") {
				...
				console.log(`${jobPrefix}${colors.green("✓")} ${colors.cyanBright(outputPath)} ${colors.gray(`· ${fileCount} ${fileCount === 1 ? "file" : "files"} · ${format(totalMs)}`)}`);
			}
		}
...
async function defineConfig$1(config) {
