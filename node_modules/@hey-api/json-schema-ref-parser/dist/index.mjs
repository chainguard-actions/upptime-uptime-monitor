	...
	*/
	...
	for (const entry of inventory) {
		...
		if (!defName) {
			...
			if (usedNamesByObj.get(container).has(proposed)) proposed = isWholeFileRef ? proposedBase : `${proposedBase}_${schemaName}`;
...
function getJsonSchemaRefParserDefaultOptions() {
	return {
		/**
...
async function sendRequest({ fetchOptions, redirects = [], timeout = 6e4, url }) {
	url = new URL(url);
