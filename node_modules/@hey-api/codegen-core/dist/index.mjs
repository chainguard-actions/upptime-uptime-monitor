...
const pythonNameConflictResolver = ({ attempt, baseName }) => attempt === 1 ? `${baseName}_` : `${baseName}_${attempt}`;
const simpleNameConflictResolver = ({ attempt, baseName }) => `${baseName}${attempt + 1}`;
const underscoreNameConflictResolver = ({ attempt, baseName }) => `${baseName}_${attempt + 1}`;
//#endregion
...
function createScope(args = {}) {
	return {
		childNames: args.childNames || /* @__PURE__ */ new Map(),
		children: [],
...
function registerName(scope, name, kind) {
	const kinds = scope.localNames.get(name) ?? /* @__PURE__ */ new Set();
	kinds.add(kind);
	scope.localNames.set(name, kinds);
}
function registerChildName(scope, name, kind) {
	const kinds = scope.childNames.get(name) ?? /* @__PURE__ */ new Set();
	kinds.add(kind);
...
var AnalysisContext = class {
	...
	*/
	...
	injectChildren(input) {
		const value = isRef(input) ? input : ref(input);
		const symbol = isSymbolRef(value) ? fromRef(value) : void 0;
		if (!symbol) return;
		for (const child of symbol.children) if (!child.overridable) registerChildName(this.scope, child.name, child.kind);
	}
	localNames(scope) {
...
var Planner = class Planner {
	...
	*/
	assignSymbolName(args) {
		...
		for (const scope of updateScopes) registerName(scope, symbol.finalName, symbol.kind);
	}
	/**
	...
	*/
	nameIsAvailable({ kind, language, name, override, scope }) {
		function conflicts(kinds) {
			if (!kinds) return false;
			for (const existingKind of kinds) if (!canDeclarationsShareIdentifier(language, kind, existingKind)) return true;
			return false;
		}
		let current = scope;
		while (current) {
			if (!override && conflicts(current.childNames.get(name))) return false;
...
var SymbolRegistry = class {
	...
	*registered() {
	...
	buildIndexKeySpace(meta, prefix = "", entries = []) {
		for (const [key, value] of Object.entries(meta)) {
			const path = prefix ? `${prefix}.${key}` : key;
			if (value && typeof value === "object" && !Array.isArray(value)) this.buildIndexKeySpace(value, path, entries);
			else entries.push([
				path,
				value,
