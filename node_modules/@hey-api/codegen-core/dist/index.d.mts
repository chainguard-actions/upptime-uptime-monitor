 ...
 */
interface IProjectMeta extends Partial<Record<Language, unknown>> {}
/**
...
interface INode<T = unknown, L extends Language = Language> {
...
type Scope = {
  /** Soft conflicts, inherited names from children symbols. */childNames: NameScopes; /** Child scopes. */
  children: Array<Scope>; /** Hard conflicts, declared names in this scope. */
...
type NameConflictResolver = (args: {
  /** The current attempt number for generating a unique name. Starts at 1. */attempt: number; /** The base name to resolve conflicts for. */
  baseName: string;
}) => string | null;
...
declare const pythonNameConflictResolver: NameConflictResolver;
 ...
 * ```
 */
declare function refs<T extends Record<string, unknown>>(obj: T): Refs<T>;
/**
 ...
 * ```
 */
declare function fromRef<T extends Ref<unknown> | undefined>(ref: T): FromRef<T>;
/**
 ...
 * ```
 */
declare function fromRefs<T extends Refs<Record<string, unknown>>>(obj: T): FromRefs<T>;
/**
...
declare class Version<TVersion extends string = string> {
