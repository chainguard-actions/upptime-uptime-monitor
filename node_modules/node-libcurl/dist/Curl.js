...
class Curl extends events_1.EventEmitter {
     ...
     */
    static defaultUserAgent = `node-libcurl/${package_json_1.default.version}`;
    /**
     ...
     * Will always be zero in case {@link CurlFeature.NoDataStorage | `NoDataStorage`} flag is enabled
     */
    headerChunksLength = 0;
    /**
     * Currently enabled features.
     *
     * See {@link enable | `enable`} and {@link disable | `disable`}
     */
    features = CurlFeature_1.CurlFeature.Empty;
    // these are for stream handling
    // the streams themselves
    writeFunctionStream = null;
    readFunctionStream = null;
    // READFUNCTION / upload related
    streamReadFunctionCallbacksToClean = [];
    // a state machine would be better here than all these flags 🤣
    streamReadFunctionShouldEnd = false;
    streamReadFunctionShouldPause = false;
    streamReadFunctionPaused = false;
    // WRITEFUNCTION / download related
    streamWriteFunctionHighWaterMark;
    streamPendingReadSize = 0;
    // common
    streamPauseNext = false;
    streamContinueNext = false;
    streamError = false;
    streamUserSuppliedProgressFunction = null;
    nextPauseFlags = null;
    /**
    ...
    setStreamResponseHighWaterMark(highWaterMark) {
        ...
        const bufferSize = highWaterMark
            ? Math.max(1024, Math.min(highWaterMark, Curl.isVersionGreaterOrEqualThan(7, 88, 1)
                ? 10 * 1024 * 1024
                : 512 * 1024))
            : 16 * 1024;
    ...
    streamModeProgressFunction(dltotal, dlnow, ultotal, ulnow) {
        if (this.nextPauseFlags !== null) {
            ...
            this.pause(this.handle.pauseFlags & ~CurlPause_1.CurlPause.Recv);
        }
        if (this.streamError)
    ...
    defaultWriteFunctionStreamBased(chunk, size, nmemb) {
        ...
        if (!this.writeFunctionStream) {
            ...
            const writeFunctionStream = new stream_1.Readable({
                autoDestroy: true,
                ...
                destroy(error, cb) {
                    ...
                    if (handle.isRunning && handle.handle.isPausedRecv) {
                        try {
                            handle.pause(handle.handle.pauseFlags & ~CurlPause_1.CurlPause.Recv);
                        }
                        catch (error) {
                            cb(error);
                            return;
     ...
     * @param z patch
     */
    static isVersionGreaterOrEqualThan = (x, y, z = 0) => {
